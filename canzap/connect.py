"""Native cognis-connect emit for canzap — forward findings to any platform.

Maps canzap's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    canzap ... --format json | canzap-emit --to stix
    canzap-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        out = dict(rec)
        out['title'] = rec.get('id_hex', '') + ' ' + rec.get('data_hex', '')
        out['severity'] = rec.get('severity', 'info')
        out['type'] = rec.get('type', 'can_message')
